...
class TestPrompts:
    ...
    def test_create_prompt(self, client: TestClient, sample_prompt_data: dict[str, str]):
    ...
    def test_list_prompts_with_data(self, client: TestClient, sample_prompt_data: dict[str, str]):
    ...
    def test_get_prompt_success(self, client: TestClient, sample_prompt_data: dict[str, str]):
        # Create a prompt first
    ...
    def test_delete_prompt(self, client: TestClient, sample_prompt_data: dict[str, str]):
        # Create a prompt first
        create_response = client.post("/prompts", json=sample_prompt_data)
        prompt_id = create_response.json()["id"]
...
def test_update_prompt(client: TestClient, sample_prompt_data: dict[str, str]):
    # Create a prompt first
    create_response = client.post("/prompts", json=sample_prompt_data)
...
def test_sorting_order(client: TestClient):
    ...
    assert response.status_code == 200  # Verify request was successful
    # Ensure the newest prompt appears first in the list
    assert prompts[0]["title"] == "Second", "Newest prompt should be the first in the list"


class TestCollections:
    """Tests for collection endpoints."""

    def test_create_collection(self, client: TestClient, sample_collection_data: dict[str, str]):
    ...
    def test_list_collections(self, client: TestClient, sample_collection_data: dict[str, str]):
    ...
    def test_delete_collection_with_prompts(self, client: TestClient, sample_collection_data: dict[str, str], sample_prompt_data: dict[str, str]):
        """Test deleting a collection orphans its prompts.

        When a collection is deleted, all prompts in that collection should
        have their collection_id set to None instead of becoming orphaned
        ...
        # Verify prompt has collection_id
        ...
        assert prompt_after["collection_id"] is None

        # Verify prompt is in the general list
        all_prompts = client.get("/prompts").json()
        assert len(all_prompts["prompts"]) == 1
        assert all_prompts["prompts"][0]["collection_id"] is None

    def test_delete_collection_with_multiple_prompts(self, client: TestClient, sample_collection_data: dict[str, str]):
        """Test that deleting a collection orphans multiple prompts."""
        # Create collection
        col_response = client.post("/collections", json=sample_collection_data)
        collection_id = col_response.json()["id"]

        # Create multiple prompts in collection
        prompt_ids = []
        for i in range(3):
            prompt_data = {
                "title": f"Prompt {i}",
        ...
        # Verify all prompts have collection_id set to None
...
class TestEdgeCases:
    ...
    def test_create_prompt_with_valid_collection(self, client: TestClient, sample_collection_data: dict[str, str]):
...
class TestSearchAndFilter:
    ...
    def test_search_by_title(self, client: TestClient):
        ...
        # Create prompts
        client.post(
            "/prompts", json={"title": "Email Template", "content": "Content"})
        client.post(
            "/prompts", json={"title": "SMS Template", "content": "Content"})
        client.post(
            "/prompts", json={"title": "Code Review", "content": "Content"})
    ...
    def test_filter_by_collection(self, client: TestClient, sample_collection_data: dict[str, str]):
    ...
    def test_combined_search_and_filter(self, client: TestClient, sample_collection_data: dict[str, str]):
        ...
        response = client.get(
            f"/prompts?collection_id={collection_id}&search=email")
        assert response.status_code == 200
        prompts = response.json()["prompts"]
        assert len(prompts) == 1
...
class TestCollectionValidation:
    ...
    def test_get_collection_success(self, client: TestClient, sample_collection_data: dict[str, str]):
...
class TestDataIntegrity:
    ...
    def test_prompt_id_is_unique(
        self,
        client: TestClient,
        sample_prompt_data: dict[str, str]
    ):
    ...
    def test_collection_id_is_unique(
        self,
        client: TestClient,
        sample_collection_data: dict[str, str]
    ):
    ...
    def test_multiple_prompts_same_collection(
        self,
        client: TestClient,
        sample_collection_data: dict[str, str]
    ):
